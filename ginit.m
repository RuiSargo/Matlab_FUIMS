...
LINC=0.05;			% 0.1m length for spline interpolation
DT=0.1;				% Sample interval for controller
TEND=500000*DT;     % Total maximum run time for simulator
VVEL=2;				% Vehicle velocity (assumed constant)


%__
KP=1;					% vehicle position error gain
KO=1.85;%1;					% vehicle orientation error gain
SIGMA_SENSOR=1;%0.3;       % Distance sigma sensor
%__
...
R_MAX_RANGE=300;%100;%400.0;% maximum range (m)
...
GSIGMA_WHEEL=0.1;%0.1; 	 % wheel variance used for control generation
GSIGMA_STEER=0.015;%0.035;	 % steer variance used for control generation
...
SIGMA_RANGE=0.3;%0.3;		% Range Variance (m)


SIGMA_BEARING=0.035; % bearing variance (rads)

fact=10; % an inflation factor to deal with sample impoverishment
