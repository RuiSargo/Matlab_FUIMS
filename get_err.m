...
% Fix oerr issue with atan2 calculating phiest not following x(3) values,
% thus giving an inaccurate oerr
if abs(phiest-x(3)) > pi 
    %fprintf('phiest=%f, ', phiest);
    phiest = phiest+(fix(abs(phiest)/pi)+2)*pi*abs(x(3))/x(3);
    %fprintf('phiest_sol=%f, x(3)=%f, ',phiest,x(3));
end
oerr=(phiest-x(3));
%fprintf('oerr=%f\n',oerr);